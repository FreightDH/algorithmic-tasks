...
function removeFirstOccurrences(str, value) {
  ...
  if (i === -1) return str;
  return `${str.slice(0, i)}${str.slice(i + spaceLength)}`;
}
...
function isPalindrome(str) {
  ...
  const temp = str.toLowerCase().replaceAll(/[,?.! ]/g, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < length; i += 1) {
    const wordLength = sentenceArr[i].length;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < length; i += 1) {
    const symbolIndex = INPUT.indexOf(str[i]);
    result += OUTPUT[symbolIndex] ? OUTPUT[symbolIndex] : str[i];
  }
